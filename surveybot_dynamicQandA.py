...
def get_response(llm,plot,prompt):
    
    #template= '''You are survey assistant so generate 1 movie survey question for user from specific plot like {context} and once you genrated the question then ask user to give feedback once feedback received from user such as {prompt} so generate new survey question based on pervious feedback , after that ask completely new survey question dont repeat same question again and again'''
    #template= '''You are survey assistant so generate 1 movie survey question for user from specific topic like {context} and ask user for feedback on question asked by you and generate next new survey question from feedback provied by user like {prompt}'''
    #template = 'You are the Movie survey Assistant,so Generate one movie survey question to the user from the provided context or movie plot like:{context} and it should always generate a new survey question based on user feedback like:{prompt} and it should not generate or ask the same survey question again to the user.'
    #template = #''' your are the movies survey assitant so ask the below question from given plot like {context} and one by one  allow user to give answer to question from {prompt} in below format \
    template = 'You are the Movie survey Assistant ,so generate one movie survey question to user from given context or movie plot like {plot} and it should always generate new survey question on the basis of user feedback like {prompt} and it not generate or ask same survey question again to user and stop asking question after two survey question and say thank you to user'
    
    prompt = PromptTemplate(input_variables=["plot","prompt"],template=template)
    
    response=llm(prompt.format(plot=plot,prompt=prompt,messages=st.session_state.messages))
